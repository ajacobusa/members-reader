...
def _route_order_impl(order: dict, recipient: dict = None, artwork_url: str = "") -> dict:
    ...
    if vendor == "gelato":
        ...
        # Preview-vs-print safety (UNCONDITIONAL): the storefront editor's design
        # (photo / wording / template / position / rotation) is NOT yet rendered into
        ...
        # clean-composite render is wired, a designed order must NEVER auto-submit a
        # poster in place of the buyer's design. Hold for manual placement. A real
        # print file (extra_files) OR an explicit faithful_artwork flag (set once the
        # composite render is wired) is the escape hatch. Scope: EVERY editor-designed
        # family - apparel AND mug AND branded (tote etc.). Mugs/totes are now genuinely
        # UID-mapped, so without this hold the first live mug order would auto-charge
        # Gelato with a generic wrap the buyer never approved (end-to-end audit gap 5).
        if (str(order.get("product_type", "")).lower() in ("apparel", "mug", "branded")
                and not order.get("extra_files")
                and not order.get("faithful_artwork")):
            return {"status": "manual", "vendor": "gelato", "id": "",
                    "detail": "designed order not yet rendered to a faithful print file "
