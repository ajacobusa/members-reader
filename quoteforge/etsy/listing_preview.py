...
<script>
 ...
 function _moveDrag(ev){{ if(!DRAGGING) return;
   const f=_frac(ev);
   if(DRAGTARGET==='photo'){{
     if(IS_APPAREL){{                                           // MOVE the photo (place it)
       PHOTO_FX=_clamp(f.x,0,1); PHOTO_FY=_clamp(f.y,0,1);
     }} else if(DRAGLAST){{                                      // pan within the framed print
       PHOTO_FX=_clamp(PHOTO_FX-(f.x-DRAGLAST.x),0,1);
