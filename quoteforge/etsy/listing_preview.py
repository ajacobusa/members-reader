 ...
 *{{box-sizing:border-box}}
 ...
 /* When frame selection is done, the whole order card becomes the active next
    step: a gold ring (via box-shadow, so no layout shift), lift, tint + badge. */
 .orderbox.stepnow{{border-color:var(--gold);
   box-shadow:0 0 0 2px var(--gold),0 10px 30px rgba(201,168,76,.32);
   background:linear-gradient(180deg,#fffdf6,#fff)}}
 .orderbox .stepbadge{{display:none}}
 .orderbox.stepnow .stepbadge{{display:inline-block;margin-left:8px;font-size:11px;
   font-weight:800;letter-spacing:.02em;color:#7a5c12;background:#fbf1d4;
   border:1px solid var(--gold);border-radius:999px;padding:2px 10px;
   vertical-align:middle;animation:badgepulse 1.3s ease-in-out infinite}}
